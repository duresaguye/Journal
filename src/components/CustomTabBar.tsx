import React from "react";
import { View, TouchableOpacity, Animated } from "react-native";
import { BottomTabBarProps } from "@react-navigation/bottom-tabs";
import { MaterialCommunityIcons } from "@expo/vector-icons";

interface Route {
  key: string;
  name: string;
}

interface Descriptor {
  options: {
    tabBarAccessibilityLabel?: string;
    tabBarTestID?: string;
  };
}

interface CustomTabBarProps {
  state: {
    routes: Route[];
    index: number;
  };
  descriptors: { [key: string]: Descriptor };
  navigation: {
    emit: (event: { type: string; target: string }) => { defaultPrevented: boolean };
    navigate: (name: string) => void;
  };
}

const CustomTabBar: React.FC<BottomTabBarProps> = ({ state, descriptors, navigation }) => {
  const scaleValue = new Animated.Value(1); // Controls shrinking effect

  const handleScroll = (scrollY: number) => {
    const scale = Math.max(0.8, 1 - scrollY / 200);
    Animated.timing(scaleValue, {
      toValue: scale,
      duration: 200,
      useNativeDriver: true,
    }).start();
  };

  return (
    <View className="absolute bottom-0 left-0 right-0 flex-row h-16 bg-gray-800 border-t border-gray-100 items-center justify-around shadow-md">
      {state.routes.map((route: Route, index: number) => {
        const { options } = descriptors[route.key];
        const isFocused = state.index === index;

        const onPress = () => {
          const event = navigation.emit({
            type: "tabPress",
            target: route.key,
            canPreventDefault: true,
          });

          if (!isFocused && !event.defaultPrevented) {
            navigation.navigate(route.name);
          }
        };

        if (route.name === "Add") return null; // Hide "Add" from normal tabs

        // Map each route name to a MaterialIcon name
        const iconName =
          route.name === "Home"
            ? "home"
            : route.name === "Library"
            ? "book"
            : route.name === "Profile"
            ? "account"
            : "alert-circle-outline"; // Default icon if no match found

        return (
          <TouchableOpacity
            key={route.key}
            accessibilityRole="button"
            accessibilityState={isFocused ? { selected: true } : {}}
            accessibilityLabel={options.tabBarAccessibilityLabel}
            onPress={onPress}
            className="flex-1 items-center justify-center"
          >
            <MaterialCommunityIcons
              name={isFocused ? iconName : (`${iconName}-outline` as any)}
              size={25}
              color={isFocused ? "#FF6347" : "gray"}
            />
          </TouchableOpacity>
        );
      })}

      {/* Floating + Button */}
      <Animated.View style={{ transform: [{ scale: scaleValue }] }} className="absolute bottom-20 right-20 z-10">
        <TouchableOpacity
          onPress={() => navigation.navigate("AddJournalEntry")}
          className="w-16 h-16 bg-orange-600 rounded-full items-center justify-center shadow-lg"
        >
          <MaterialCommunityIcons name="plus" size={32} color="white" />
        </TouchableOpacity>
      </Animated.View>

      {/* Floating Microphone Button */}
      <Animated.View style={{ transform: [{ scale: scaleValue }] }} className="absolute bottom-20 right-5 z-10">
        <TouchableOpacity
          onPress={() => console.log("Voice input triggered")} // Replace this with your voice recording function
          className="w-16 h-16 bg-blue-600 rounded-full items-center justify-center shadow-lg"
        >
          <MaterialCommunityIcons name="microphone" size={32} color="white" />
        </TouchableOpacity>
      </Animated.View>
    </View>
  );
};

export default CustomTabBar;
